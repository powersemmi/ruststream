...
Когда сервис растёт, обработчики переносятся из `main.rs` в собственные модули. `Router` собирает
...
выбирает само определение; рядом стоит `with_codec`, он переключает кодек декодирования для цепочки
(см. [Кодеки](codecs.md#per-handler)). Источник подписки всегда приходит из определения -
`#[subscriber(..)]` берёт выражение-источник самого брокера вместе с цепочкой билдера, - так что в
точке монтирования называть нечего. Каждый вызов поглощает роутер и возвращает новый, поэтому
регистрации выстраиваются в цепочку:
...
области, с одним отличием: регистрация фиксируется явным терминалом. `.out(marker, policy)` задаёт
политику публикации одной позиции - `Reply` для ответа, маркер слота `Out` для слота, - а
`.build()` фиксирует регистрацию и берёт собственную политику публикации брокера по умолчанию для
ответа, которому `.out(Reply, ..)` не назвали. Забытый терминал никогда не станет роутером, поэтому
цепочка не скомпилируется. Политики остаются чистой декларацией, поэтому роутеру по-прежнему не
нужен брокер:
