...
определение. `with_codec` переключает кодек декодирования для следующих за ним регистраций, а уже
смонтированные сохраняют свой (см. [Кодеки](codecs.md#per-handler)).

Источник подписки задаёт само определение: `#[subscriber(..)]` берёт выражение-источник брокера
вместе с цепочкой билдера, поэтому в точке монтирования источник не указывают. Каждый вызов
...
`.out_reply(..)` не указан, `.build()` берёт для ответа политику публикации брокера по умолчанию.
