...
определения (обычную, сырую, batch, с публикацией ответа, с внедрением), а какую именно - выбирает
само определение; рядом стоят `with_codec` (переключает кодек декодирования для цепочки, см.
...
`.out(marker, policy)` привязывает один именованный слот перед `.mount()`. Потребляющий билдер не
может зафиксироваться сам, выходя из области видимости, как это делает билдер области: при сбросе
ему некуда вернуть роутер, в который выросла регистрация, - поэтому забытый терминал никогда не
станет роутером, и цепочка не скомпилируется. Политики остаются чистой декларацией, поэтому роутеру
по-прежнему не нужен брокер:
