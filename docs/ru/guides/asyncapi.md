...
Куда уйдёт ответ, можно решать на каждой доставке: преобразование на позиции ответа читает заголовок
reply-to запроса и само называет адресата. Тогда у канала нет постоянного адреса, и вместо него
операция говорит, откуда клиенту этот адрес взять.
...
Выражение принадлежит брокеру: оно называет заголовок, через который отвечают его собственные
клиенты. Брокер, который через заголовок не отвечает, поле опускает, и ответ остаётся на имени,
объявленном в точке монтирования.
...
То, что сервис публикует, называет свой media type так же, и берётся он из кодека, привязанного в
точке монтирования к этой позиции. Ответ, закодированный через `.out_reply(policy).codec(CborCodec)`,
сообщает `application/cbor`, а запрос, на который он отвечает, остаётся `application/json`; корневое
`defaultContentType` тогда исчезает, потому что эти два расходятся, а корневое поле заявляет один
формат на весь документ. Недоставленное сообщение несёт те же байты, с которыми пришло, и сообщает
media type своей подписки. Ответ или запись слота на полосе `Serialized` не сообщает ничего, по той
же причине, по которой молчит вход `Deserialized`: его никто не кодировал.
