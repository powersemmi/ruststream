...
Тип сообщения, чьё объявленное имя является шаблоном (`#[outgoing(name = "orders.{tenant}.v1")]`),
объявляется по этому шаблонному адресу, а блок **parameters** канала заполняется из его подстановок,
поэтому декларация и точка вызова не могут разъехаться. Тип, который не объявляет адресата, не даёт и
канала - ровно то, что он о себе и сообщает. См.
...
Крейт брокера может реализовать и возможность `DescribeServer` - тогда спецификацию отдаст
...
Безопасность - заявление автора сервиса, а не брокера: один и тот же брокер разворачивают наружу и
внутрь с разной аутентификацией, поэтому `DescribeServer` о ней никогда не сообщает. Чтобы закрыть
сервер, который брокер зарегистрировал автоматически (`with_broker_labeled`), объявите его явно:
