...
Раз диспетчеризация живёт в сгенерированном бинарнике, `ruststream run` и обычный `cargo run -- run`
запускают сервис одинаково. `ruststream run` - это удобная обёртка: она находит крейт и передаёт
команду в `cargo`.
...
[быстром старте](../getting-started/quickstart.md). Шаблон принадлежит тому крейту, чей брокер он
