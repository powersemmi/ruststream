...
[W3C Trace Context](https://www.w3.org/TR/trace-context/) и порождает спаны `tracing`; оно не
...
за `Otel::builder().init()`, который ставит глобальные провайдеры и перекидывает в них спаны. Либо
...
несёт новый `span-id` (спан потребителя), поэтому трасса связана из конца в конец. Доставка без
`traceparent` начинает новую корневую трассу, помеченную как сэмплируемая. Спаны уходят под целью
...
Модуль распространения останавливается на W3C-контексте и спанах `tracing`; доставить их в коллектор
...
последние спаны и точки. Чтобы вписать мост спанов в свой стек подписчиков (например, вместе со
