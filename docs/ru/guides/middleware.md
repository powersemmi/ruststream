...
!!! note "Чтобы дотянуться до обработчиков роутера, нужен `BlanketLayer`"
    Роутер скрывает конкретные типы своих обработчиков, поэтому слой, который их оборачивает (стек
    приложения на `include_router` или `Router::layer`), обязан реализовать `BlanketLayer` - один
    обобщённый метод, оборачивающий любой обработчик. Поставляемые слои его реализуют; для своего
    слоя это несколько строк рядом с его impl `Layer` (см. `LogLayer` в примерах выше).
...
## Почему middleware по умолчанию статическое

Слои выше разрешаются во время компиляции: `with` и `layer` строят конкретный вложенный тип
обработчика (`Logged<Typed<..>>`), а `Handler::handle` возвращает `impl Future`, тип которого
известен. Компилятор мономорфизирует всю цепочку в один автомат состояний и инлайнит через границы
слоёв, поэтому статический слой не добавляет ни стоимости диспетчеризации, ни аллокаций - это
zero-cost абстракция.

Динамическая (`dyn`) цепочка от этого отказывается. `Handler::handle` - это `async fn in trait`,
поэтому его future анонимный, `impl Future`, а трейт, возвращающий `impl Trait`, не object-safe.
Чтобы держать middleware за `dyn`, future приходится боксить (`Pin<Box<dyn Future>>`): одна
аллокация в куче на слой на каждое сообщение, и вызов уже нельзя ни заинлайнить, ни специализировать
через границу `dyn`. Связка `dyn` + `async` не оптимизируется, так что эта цена легла бы на каждый
обработчик - при том что цепочка почти всегда известна во время компиляции. Отсюда и статическое
...
`next.run(..)`, чтобы продолжить, либо коротко замыкает цепочку собственным результатом. Он
object-safe, поэтому боксированный future возвращает явно:
...
написанный руками. Остальная цепочка диспетчеризации остаётся статической, а за боксинг платится
только внутри стека:
...
внешнего. Каждый динамический слой стоит одного боксированного future на вызов - против нуля у
статических слоёв, - поэтому держите статическую цепочку как вариант по умолчанию и беритесь за
`DynStack` только там, где сборка в рантайме себя окупает.

## Middleware на стороне публикации
