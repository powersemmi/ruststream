...
Вместо всего приложения можно обернуть одну регистрацию: `.layer(..)` после `include` на роутере
действует на этой регистрации - ровно так же, как остальные шаги цепочки действуют на позиции, названной
перед ними.
...
Это то, что нужно, когда слой требуется только части обработчиков, и это единственное место, куда
можно положить слой, не являющийся `BlanketLayer`: тип обработчика регистрации здесь ещё конкретен,
поэтому достаточно обычного `Layer<H>`. Слой стоит снаружи шага декодирования, поэтому видит сырую
доставку, и сочетается со стеком приложения и стеком роутера.
...
обычным статическим `Layer`, но привязанным к одному типу входа, поэтому он подключается к одной
регистрации через `.layer(..)`, а не в стеке приложения, который принимает только blanket-слои.
Остальная цепочка диспетчеризации остаётся статической, накладные расходы есть
...
`DynStack<I>` обобщён по входу, который оборачивает. На регистрации он оборачивает целиком
...
годится для любого из уровней. Middleware внутри одного `DynStack` выполняется в порядке списка,
начиная с самого внешнего. Держите статическую цепочку как вариант по умолчанию и беритесь за
`DynStack` только там, где сборка в рантайме себя окупает.
