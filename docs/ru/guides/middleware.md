...
слоёв, поэтому статический слой не добавляет ни стоимости диспетчеризации, ни аллокаций.
...
чтобы держать middleware за `dyn`, future приходится боксить (`Pin<Box<dyn Future>>`). Это одна
