...
Публикация, подтверждение и, если доставка это умеет, `nack_after` выполняются в рантайме с одним
потоком на отдельном потоке, и этот рантайм останавливается раньше, чем проверка идёт дальше.
Отложенное сообщение обязано вернуться, когда задержка истечёт: так проверка требует, чтобы
внутренние задачи брокера выполнялись в рантайме, где он подключился (см.
[Как написать брокер](index.md)). Издатель и доставка переходят в тот поток, поэтому оба
`'static`.
