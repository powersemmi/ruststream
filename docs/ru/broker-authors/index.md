...
[его даёт каждый брокер](#batches-batchsubscriber), потому что его просит каждый пакетный
обработчик.

| Трейт | Для брокеров, которые умеют |
|---|---|
| `BatchSubscriber` | получать сообщения пакетами (каждый брокер, см. ниже) |
...
Срок, по которому закрывается неполный пакет, выбираете вы - один раз, когда строите обёртку;
...
роняет брокер, чьи пакеты приходят длиннее.
