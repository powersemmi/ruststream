...
[примере с NATS](example-nats.md). [Обвязка conformance](conformance.md) проверяет этот жизненный
...
поддерживает; ни одна из них не входит в обязательный интерфейс.
...
Тело обработчика ни один из этих трейтов не называет. Оно ограничивает свой слот типизированным
близнецом, который ядро выводит из капабилити: `Publish` для `Publisher`, `TransactionalPublish`
для `TransactionalPublisher`, `OwnedTransactionalPublish` для `OwnedTransactions`,
`RequestReplyPublish` для `RequestReply`. Политику точки включения ядро проверяет против трейта
брокера один раз, там, где слот привязывается, а близнец даёт телу типизированные операции этой
капабилити поверх кодека и словаря записи. Реализуйте трейт брокера на живом паблишере; близнец
выполняется сам, и именно его имя сервис пишет в `Out<impl ..>`.

### Расширение словаря слотов `Out`

Параметр обработчика `Out<impl X, Marker>` принимает любой `X`, который реализует обёртка
`SlotPublisher` из рантайма; ядро делегирует собственный набор совместимостей (`Publisher`,
`TransactionalPublisher`, `OwnedTransactions`, `RequestReply`). Когда соединённое вами значение
умеет больше - или вовсе не издатель (кэш продюсеров по партициям, шардирующий
роутер), - объявите собственную трейт-совместимость, реализуйте её для этого значения и привейте к
