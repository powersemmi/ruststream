...
[примере с NATS](example-nats.md). [Обвязка conformance](conformance.md) проверяет эту лестницу от
начала до конца.
...
подключённой форме ещё и `DefaultPublish`, чтобы её назвать. Тогда рантайм собирает публикатор
ответа по умолчанию, когда обработчик с `publish("dest")` монтируется без явного
`.publisher(..)`: `b.include(def)` компилируется сам по себе. Брокеры, публикаторам
