...
Тело обработчика держит не само значение, а запись арены `Slot<Marker, W, E, Body>` - прозрачное
...
Добавьте рядом со своим трейтом один blanket-impl - `impl<M, W: Lanes, E, Body> Lanes for
Slot<M, W, E, Body>`, делегирующий через `Deref` записи, - и хелперы и тела, обобщённые по
