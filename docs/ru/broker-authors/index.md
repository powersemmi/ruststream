...
Что бы вы ни называли в собственном prelude, держитесь в стороне от имён, которые экспортирует
`ruststream::prelude`. `Publish`, трейт слота, который называет граница обработчика, - одно из
них, а сервис пишет оба glob-импорта рядом. Явный ре-экспорт побеждает glob, поэтому
`pub use crate::MyPublish as Publish;` в вашем prelude молча подменяет трейт вашей
структурой-политикой, и собственная `fn f<T: Publish>()` сервиса падает с
`E0404: expected trait, found struct`, указывая на код, который ни в чём не виноват. Политики
сохраняют имена с префиксом (`MemoryPublish`, а не `Publish`). Закрепите это пробой, которая
перестаёт компилироваться, как только имя перекрыто:
