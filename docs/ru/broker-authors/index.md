...
`shutdown` нельзя ни блокировать, ни доводить до паники; всю падающую разборку делайте здесь и
возвращайте `Result`. У свидетеля `Closed` нет ни поверхности публикации, ни подписки; складывайте
в него диагностику остановки (результаты сброса буферов, счётчики потерь) как обычные данные - или
возьмите `()`.
...
[примере с NATS](example-nats.md). Именно этот контракт позволяет сервису собираться синхронным
билдером `#[ruststream::app]`; [обвязка conformance](conformance.md) проверяет его от начала до
конца.

Потребляющие переходы делают ошибку со стороны владельца непредставимой: у брокера, который вы уже
остановили, нечего вызвать - ни публикации, ни подписки. Рантайм-правилом остаётся то, что диктует
реальность транспорта, а не бухгалтерия контракта: хендлы, разделяющие соединение (публикаторы,
розданные из подключённой формы, клоны разделяемого брокера), обязаны возвращать ошибку при
использовании после остановки - и никогда не отрабатывать молча и успешно на мёртвом соединении.
Проверка `lifecycle` проходит и по этому пути.
...
Два метода с реализацией по умолчанию - это способ мягко деградировать необязательному поведению
брокера: брокер, который не переопределил ни одного из них, всё равно работает со всеми
возможностями рантайма - `retry_after` сводится к немедленному возврату в очередь, а полосы
воркеров по ключу раскладывают сообщения без ключа по кругу.
...
Брокер реализует `publish` и получает весь билдер даром - предоставлять больше нечего, и на билдере
нет ничего, за чем крейту брокера пришлось бы поспевать.
...
его с подключённой формой в живой публикатор. Соединение асинхронное и падающее - ради брокеров,
которые делают настоящую работу, когда публикатор оживает (инициализация транзакционного
producer'а); для большинства это дешёвый вызов конструктора.
...
`PairError::new`), а не `C::Error`: соединение выполняется по одному разу на публикатор на старте и
никогда не попадает на горячий путь, а кросс-брокерный токен соединяется с брокером, отличным от
брокера включающей области, - так что ошибка, типизированная брокером, всё равно не смогла бы
назвать какой-то один брокер.
...
имени (топик *и* имя подписки), его не реализует, и такая форма для него не компилируется - это
честная граница, и она избавляет дескриптор от `Default`, который сделал бы представимым
...
Трейт локален для вашего крейта, поэтому orphan rule соблюдено, а ограничение по типу источника
означает, что на билдере другого брокера этих методов просто не существует. Пользователи
...
| `OwnedTransactions` / `Transaction` | транзакции, чей буфер живёт в значении: на один хендл их может быть открыто сколько угодно |
...
умеет больше - или вовсе не является публикатором (кэш producer'ов по партициям, шардирующий
...
Публикации, сделанные через значения, полученные из `inner`, проходят мимо пер-слотового захвата
обвязки (как буфер уже завершённой owned-транзакции); в логе публикаций брокера они остаются видны.
...
`RustStream::publish_layer`. Слой публикации асинхронный и падающий, а `Outgoing::payload_mut`
существует ровно для оборачивания в конверт.

## Конфигурация и умолчания

Свой `Config` принадлежит вашему крейту. Ядро не несёт никакой специфичной для брокера
конфигурации - именно это удерживает изменение в апстриме в границах одного крейта брокера. Если у
поля конфигурации нет разумного умолчания, не реализуйте для него `Default`: пусть пользователь
задаст значение явно, чем вы поставите умолчание, которое потом сломается.

## Ошибки

Возьмите `thiserror` и один enum ошибок на весь крейт, с вариантами по источнику. Публичные enum'ы
ошибок помечайте `#[non_exhaustive]`. В библиотечном крейте `anyhow` не используйте никогда.
...
семантику (durable-курсоры, таймеры повторной доставки, offset'ы, маршрутизацию в dead-letter) - её
