...
В этом учебнике мы с нуля соберём сервис заказов и разберём каждую его часть. Он работает на
...
## 2. Описываем сообщение и обработчик

Обработчик - это `async fn`, первый параметр которой - раскодированная полезная нагрузка. Макрос
...
## 7. Переходим на настоящий брокер
