...
Регистрация на роутере завершается явным терминалом. `.out(Reply, ..)` задаёт политику публикации
ответа - политика остаётся чистой декларацией, поэтому роутеру по-прежнему не нужен брокер, - а
`.build()` фиксирует регистрацию и берёт собственную политику брокера по умолчанию, если её никто
не назвал: явная запись того, что шаг 4 получал сам. Остальная поверхность роутера разобрана в
разделе [Роутинг](../guides/routing.md).
