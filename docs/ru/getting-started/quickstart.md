...
Быстрее всего получить работающий сервис, сгенерировав заготовку через `cargo generate`.
...
++ctrl+c++ (команда CLI `ruststream run` - это удобная обёртка, которая делает то же самое).
