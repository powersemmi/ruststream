...
路由器上的注册要以一个显式的终结调用收尾。`.out(Reply, ..)` 指定回复的发布策略 - 策略是纯粹的
声明，所以路由器依旧不需要 Broker - 而 `.build()` 提交这次注册，在没人指定策略时采用 Broker 自带的
默认策略，也就是把第 4 步自动拿到的那一份显式写出来。路由器的其余接口参见[路由](../guides/routing.md)。
