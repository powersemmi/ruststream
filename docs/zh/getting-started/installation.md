...
!!! note "Edition 与 MSRV"
    ...
    `async fn in trait`）。在 `Cargo.toml` 里设置 `edition = "2024"`。CI 会在该下限版本和当前
    stable 上构建并测试本 crate，并在 beta 上构建，因此任何不低于 1.85 的下限都能用。
    当 Broker crate 底层的客户端需要更新的工具链时，它们可能要求比核心更高的版本；查看该 Broker
    crate 自己的 `rust-version`。

## Features

核心的各个 trait、`RustStream` 应用对象、`Router`、中间件和分发始终参与编译。除此之外的一切都是可加、
需要显式启用的 feature。
...
可选的 `ruststream` 二进制程序随 crate 一起发布，位于 `cli` cargo feature 之后，它用框架的子命令
