...
- **每个场景都是用户写的那个服务**，由真实运行时启动，测试用的 harness 不参与编译，测的因此
  就是发布出去的代码。数字是绝对值，即框架自身的开销。与别人写的客户端相比，是第一张表的事，
  由 Broker crate 来做。
...
- **每个场景跑三次，各有各的用处。** 一条消息、一千条、两千条。后两次之差是服务跑起来之后一条
  ...
  内存分配计数器也成立 —— 它根本没有开关。
- **每个场景三个数字。** callgrind 给出的指令数是精确的，用来对照；DHAT 给出的内存分配次数是
  精确的，由场景声明的下限把住；单独一次运行给出的时钟时间有噪声，仅供参考。
- **是对照，不是门槛。** `just bench --baseline=main` 把一个分支与目标分支上同一套基准测试的
  结果对照：某个场景的指令数多出两个百分点以上算失败，超出该场景声明的内存分配次数同样算失败。
  pull request 的作者自己运行它并引用它的数字，而降低了某个场景的改动，也在同一个 pull request
  里降低它声明的下限。冷启动和时钟时间只打印出来。
...
第一条消息的全部开销，没有除以任何东西。`gated` 说明该场景出现回归时 `just bench --baseline=main` 是否失败。只公布
