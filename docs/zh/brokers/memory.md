...
`memory` feature 下的 `MemoryBroker` 是一个完整的进程内 Broker：发布的消息按主题存入日志，和
Kafka 一样。当队列只属于单个应用而不属于网络时，选它即可。默认的 `cargo generate` 模板
（`templates/memory`）就建立在它之上，因此新建的项目没有外部依赖就能运行。
...
`MemoryPosition`，以及上下文键 `MemoryContext` / `MemoryBatchContext` / `Position` /
...
- **日志定位。** `MemorySubscriber` 在每个主题的发布日志之上实现了 `Seekable`：开始读取之前先取得
  `MemorySeeker`，再对某个 `MemoryPosition` 调用 `seek`。位置可以从已投递的消息上取得
  （`Positioned::position`，此时那一条消息会重新投递），也可以直接构造（`MemoryPosition::start()`
  / `sequence(n)`）。向前定位会跳过排在目标之前的投递；定位到日志末尾或更靠后，会跳过此前发布的
  全部消息。定位作用在单个订阅者实例上。通过已停止总线的句柄定位，返回 `MemoryError::ShutDown`
