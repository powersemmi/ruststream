...
一个 crate 应当如何提供与自身 API 保持同步的
[`cargo generate`](https://github.com/cargo-generate/cargo-generate) 骨架。这份契约是
[conformance 校验套件](conformance.md)在骨架生成这一侧的对应物：模板是由 CI 编译的产物，归它所接线的
那个 Broker 所属的 crate 所有，而不是核心 CLI 里手工维护的一堆字符串。核心只提供内存版的
...
失败，也就是让问题暴露在该修它的地方，而不是暴露在用户的第一次 `cargo build` 上。这个防漂移任务会：
...
- 每个 Broker crate 拥有自己各种传输方式的模板，并在自己的 CI 里跑那个防漂移任务。
