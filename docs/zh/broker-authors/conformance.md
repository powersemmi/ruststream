...
有一类传输根本无法确认 - ZeroMQ、MQTT `QoS 0`、Redis pub/sub、Core NATS - 它们的 `ack` 和 `nack`
返回 `AckError::Unsupported`。套件在每一处结算投递的地方都接受这个答复，于是你的进程内传输可以
和真实传输答得一模一样，而不必声称一次生产中根本不会发生的结算。只有重新投递那个场景例外：
`nack(requeue = true)` 答 `Unsupported`，场景就到此为止 - 一个什么都不收回的传输，没有重新投递
可供观察。其余断言照旧，丢弃场景也在内：没人能结算的投递，同样不该再回来。

这个答复读自投递，而不是读自 Broker，所以它可以像你的传输那样逐个订阅、逐条消息地不同：重新入队
在一种提交模式下只是建议，在另一种下会回拨位点；确认在一个服务质量等级上能给，在另一个上给不了。
不变的是「成功」的含义 - `nack(requeue = true)` 答 `Ok(())`，就是承诺这条消息会回来，运行时的
重试路径也照这个含义来读。

这些是核心路由方面的保证，是每个 Broker 都必须满足的契约。该校验套件**不会**测试 Broker 专有的语义
（持久化续传、超时重新投递、分区分配）；那些不属于该契约，要由你自己针对真实服务器的端到端测试集来验证。

它同样一项能力也不碰。每项能力在下面都有自己的套件，由你的 crate 去调用它实现的那几个 - 其中
包括 `capabilities::batches`，那是唯一一处检查「一批永远不会比开启时给的尺寸更大」的地方。
把它并进来这条路走不通：在 `run_suite` 上加一条 `BatchSubscriber` 边界，会把每个不声明该能力的
Broker 挡在门外，何况你的批本来也不是由进程内传输组装的。所以，只跑了 `run_suite` 的 crate
并没有检查过自己的批。
...
每个套件都在每次运行时重新取一个主题，而不是固定一个，因此一次运行只读到自己发布的东西。固定的
主题在全新服务器上能过一次，第二次就会栽在任何保留前一次残留的 Broker 上：留存的日志会把两次运行
一起重放进同一个订阅，持久队列里还压着早先的消息，键空间里还留着早先的类型。套件为此调用
`conformance::helpers::unique_subject`；你自己的端到端测试集有同样的麻烦，也有同样的解法。
