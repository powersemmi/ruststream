...
读。覆盖了这个方法，Broker 自己的计数就成了上限唯一读的那个：框架的消息头不会再加进来。

延迟由你自己发布一份副本来兑现时 - 死信交换机后面的一条等待队列，或者一个专门的重试主题 - 请在那份
...
**真实 Broker 保留的，进程内也要保留。**已经存在的队列或日志会留住订阅者出现之前发布给它的消息，
之后才打开的订阅会先收到这些消息。由订阅自己声明的队列在那次发布时还不存在，服务器会丢弃消息，
这时的答复是 `Backlog::Missed`。进程内把它们丢掉，一个在生产者之后才启动的服务，测试照样通过，生产中却
要处理测试从没见过的积压。声明这种行为的办法是让 `TestableBroker::backlog` 返回
`Backlog::Delivered`，`run_suite` 会检查你的传输确实投递了这条消息。默认值是 `Backlog::Missed`，
也就是发布/订阅的答复：订阅只收到它打开之后发布的消息。答复取决于某项配置时（日志从最早的偏移量
读起，流消费者投递全部消息），就从已连接形态上读取这项配置。`in_process::backlog_matches_server`
会拿这个声明去对照你的服务器（见 [Conformance](conformance.md)）。
