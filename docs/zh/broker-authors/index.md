...
### 怎样写一个信得过的替身 { #writing-one-you-can-trust }

一个服务的整套测试都跑在这个替身上，所以它和真实传输之间的每一处差异，都是一条为生产环境并不存在的
行为开出的绿灯。这些差异并不冷僻，而下面每一条规则的代价大约就是一个测试。

**把核心的契约套件也跑到替身上，而不只是跑到服务器上。**套件是照着 trait 写的，并不在乎线的另一端
是谁在应答，所以替身可以放进真实 Broker 用的同一套装置里，代价是一个 `#[tokio::test]`：
...
几乎没人拿去问替身的问题：关闭之前创建的发布者，关闭之后会不会失败？真实客户端答的是「未连接」；而
一个发布就是往 channel 里塞一条的替身没有理由失败，它会照单全收。
...
**提供的能力面要和真实 Broker 对齐。**`testing` 这个 feature 是给测试用的，release 构建会把它关掉，
两个方向的代价因此并不对等。少一项才是贵的：真实 Broker 有、替身没有的能力，在进程内根本挂载不了，
它背后的行为也就没人测。多一项则是小事：只有替身提供的事务或 request-reply，在你自己的 release
构建里就编译不过，恼人，但发现得早。

**传输怎么结算，你就怎么结算。**真实的 `ack` 返回 `AckError::Unsupported` 的地方——发完即忘的传输、
至多一次的服务质量——替身也照样返回。为了让套件闭嘴而回一个 `Ok(())`，正是让一个返回
`HandlerOutcome::retry()` 的处理器在进程内通过、却在生产环境丢消息的做法。诚实的答案套件是收的。

**客户端做的事要复刻，Broker 做的事不要假造。**这条界线无关工作量，只关乎行为长在哪一侧。竞争消费、
按组分发、关联与回复路由、提交前的缓冲，都在客户端一侧或路由层，进程内复刻是精确的；集群原子性、
fencing、Broker 侧持有的超时和 exactly-once 在 Broker 一侧，进程内复刻就是虚构。

竞争消费是最该做对的一条，因为做错了看着像成功：把每条消息发给队列的每个订阅者，那是扇出，不是队列。
共用一条队列的两个 worker 于是各自跑完了整个流，而一个统计处理量的测试只会看到活干完了，一个错都不报。

**每一处缺口都配一句注释，点名它让哪条断言站不住**——不是「这个功能没有」，而是读者从此不该再信哪个
测试，以及真正覆盖它的是什么：
...
**再用一个测试把缺口钉住。**注释会在某人「修好」替身、让它去路由那些它故意不路由的东西的那天失效；
而一个断言处理器*没有*被调用的测试，会在那天挂掉，并且自己把话说清楚。

**用生产的挂载方式来挂替身。**你自己的订阅来源和发布策略必须原封不动地能对着它工作，这样服务测的
才是它真正发布的那份路由文件。如果用户非得把 `OrdersStream` 换成别的东西才能把测试跑起来，那这个
测试就不再覆盖挂载了。
