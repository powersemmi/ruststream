...
`u32` 字段，写回时同样如此。schema 则依旧描述逻辑类型，这正是 AsyncAPI 文档化消息头的约定。
...
契约把这个位置填掉一次，而它仍然就是调用点关于消息头所说的全部。发布者自己补上的那部分走在下面：
一个为每条发出的消息都携带同一个参数的发布者，把该参数作为底交出来，契约的字段再逐个序列化到这层
底之上。带契约的消息因此也捎上了 Broker 的参数，参见
