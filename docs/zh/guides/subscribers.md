...
`ApproximateReceiveCount`、Pub/Sub 的 `delivery_attempt`），不记的时候取自框架的重试计数消息头。
两者不会同时算：会记次数的 Broker 为自己所有的重新投递负责；它的计数在某套延迟方案上不增长，那是这
个 Broker 的行为，由它自己的文档说明。

原生支持延迟重新投递的 Broker，只在这次投递还没到上限时才拿到延迟。计数先读出来，到了上限的那次投
递会发往死信地址；没有声明地址时它被拒绝，而不是再回来一次。两类传输上都是如此：自己记投递次数的那
一类，以及什么都不记、由 crate 用一条等待队列把带着消息头的延迟副本送一圈的那一类。
...
也编译不过：错误会报出这个描述符，以及让发布者失去意义的那套机制。这样的订阅上，立即重试
`retry()` 就是 Broker 自己的重新入队，有没有上限都一样：Broker 马上要搬走的这条投递，运行时不会
替它落定。
...
这条路径上的处理器用同一个 `publish` 子句回复，和其他任何回复形态一样。传输方式由回复的
