...
- 在不原生支持的 Broker 上，运行时会安排一次**延后重新发布**：等 `delay` 过去之后，把这条消息重新
  ...
  完全没有结算能力的传输（MQTT 的 QoS 0、ZeroMQ、Redis pub/sub）走的也是这条路径：没有原件可丢，
  重试就只剩这份延后的副本。Broker 主动拒绝结算是另一回事：这条投递仍归 Broker，由它自己重新投递，
  运行时只把这次失败报出来，不再往上叠一份副本。
...
这条路径上的处理器，回复用的是和其他回复写法完全相同的那一个 `publish` 子句；用哪种传输方式
