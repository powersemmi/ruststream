...
- 上下文每批一个，其中来自 Broker 的字段是*订阅级*的那些：批量函数体写出 Broker 的批量上下文类型
  ...
  键。这个值由运行时按批构造一次，取自该批的第一次投递，走的是 Broker 的 `BuildBatchContext`
  实现；订阅级上没有东西可交的 Broker 什么都不用实现，批量就停在 `()` 这个默认值上。
...
当载荷根本不是一个序列化后的值时（一个二进制帧、一种你自己解析的外部线上格式），由载荷类型自己
完成反序列化，编解码器就此退出这条路径。走哪一条由类型自己决定，而助记法就写在 trait 的名字里：
`Deserialize` 表示由框架的编解码器构造这个值，`Deserialized` 表示类型自己构造自己。
`#[derive(Deserialized)]` 覆盖了常见的、包在 `&'a [u8]` 外面的 newtype，而把参数写成
`&Frame<'_>` 就把处理器放到了这条路径上：字节和 Broker 交过来时一模一样，直接借用它的缓冲区，
不发生任何拷贝，路径上也没有编解码器。
...
不用宏时写出来的，就是 derive 会写的那一对 impl：`Deserialized` 负责构造，`Input` 那一处写法把
类型接到这条路径上。光秃秃的 `&[u8]` 参数根本不是处理器的输入，编译错误会点明 derive 才是该用的
写法：一个字节切片同时也可以是一批解码后的 `u8` 元素，只有具名类型才能把两者分开。
...
来报告坏载荷，这次投递由 `on_failure(decode = ..)` 结算：和编解码器解码失败落在同一级，也和类型化的
`Headers` 契约落在同一级。提取器、`&mut Context`、`workers(..)`、
`on_failure(panic = ..)`，以及注入的 `Out` 参数，在单次投递的形态上照常可用（一批帧
目前还不接受 `Out`）；这样的订阅者也和其他任何定义一样，用同一个 `include` 挂载，
而作用域上即便设了编解码器，对它也单纯不生效。它也是在一个编解码器 feature 都没启用时
唯一可用的订阅者写法。如果某种自定义的序列化格式仍然希望用*类型化*的处理器来写，就实现
[`Codec`](codecs.md)，继续走类型化路径。

这条路径上的处理器，回复用的是和其他回复写法完全相同的那一个 `publish("dest")` 子句；发出去时走
哪一条线，由回复的*类型*来选，助记法与输入侧互为镜像。实现了 `serde::Serialize` 的回复由回复的
编解码器编码；带 `#[derive(Serialized)]` 的 newtype 自带字节（`fn bytes(&self) -> &[u8]`），
按字节原样发出，和处理器返回时一模一样。直接返回它，或者写成
`Result<Export, HandlerOutcome>`，后者提供与编码回复写法相同的显式 ack 控制。发布者来自挂载点：
`Serialized` 回复附加一个普通的发布策略（`b.include(relay).publisher(MemoryPublish)`），而编码的
回复把同一个策略包进 `TypedPublisher::new(..)`；不写该调用时，回复就用 Broker 的默认发布策略提交。
在不用宏的路径上，两条线走的是同一条 `.reply().to(dest).publisher(..)` 链，道理相同：回复的类型
早已说明了它走哪一条。回复发布失败会让这次投递 nack 并重新入队，和编码路径上完全一样：
...
`Serialized` 回复就是网关式的形态 - 消费结构化的消息，发出由处理器自己产生的线上格式 - 此时输入
...
`Serialize` 值，因此会被编码后发出，非要原样发走的载荷仍然需要那个 newtype。另外，`Serialized`
的*批量*回复并不存在：批量的回复一律经由回复的编解码器发布。
