...
任何需要 `async` 的工作（连接那个连接池、干净地关掉它）都放进钩子里。运行循环由四个钩子夹住：
...
这个句柄带 `#[must_use]`：不调用 `shutdown` 就把它丢弃，会让服务脱离管理，不做任何优雅拆除。`run`
和 `run_until` 都建立在同一条 start/shutdown 路径之上，因此这三种写法共用同一套启动与关闭流程。

## 关闭超时 { #shutdown-timeout }

默认情况下，关闭被触发之后 `run` 会无限期等待处理中的处理器结束。用 `shutdown_timeout` 给这个等待
