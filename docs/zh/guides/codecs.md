...
`msgpack`。当没有任何地方指定编解码器时，`include(def)` 和停在 `.publisher(policy)` 的回复链用的
就是它；这两者都不接收编解码器参数。
...
发布者遵循同样的规则：`.publisher(policy)` 用默认编解码器编码回复，
`.publisher(policy).codec(codec)` 则显式指定一个。传入请求的解码遵循所在作用域（用
