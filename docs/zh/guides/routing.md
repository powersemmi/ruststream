...
Broker 自带的默认发布策略，`.out(marker, policy)` 则在 `.mount()` 之前绑定一个具名槽位。消费式的构建器
无法像作用域那样在离开作用域时自动提交，因为丢弃它并不能把注册所长成的那个路由器交还回来，所以忘记写
终结调用就永远得不到路由器，整条链也无法通过编译。这些策略仍然是纯粹的声明，因此路由器依旧不需要
Broker：
...
路由器可以携带自己的层栈：`Router::layer` 会在该路由器被挂载时包住其中的每一个处理器。应用的全局层栈
...
`merge` 会按顺序把另一个路由器的注册追加进来。每个路由器保留自己的编解码器和层栈；结果被挂载时，外层
路由器的层（以及应用的全局层栈）会包在被合并进来的那个路由器自己的层外面。
