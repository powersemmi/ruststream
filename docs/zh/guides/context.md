...
状态类型在编译期就会被检查：读取状态的 `#[subscriber]` 处理器把它写成 `Context` 的第三个泛型参数
...
也没有向下转型。服务跑起来之后状态被放在 `Arc` 后面共享，因此处理器拿到的是廉价的共享引用而不是
副本；如果某个共享值必须在运行时改变，内部可变性（`AtomicU64`、由互斥量保护的 map）才是对应的工具。
如果数据只属于一条消息而不是整个服务，请改用[按投递的上下文](#per-delivery-context)。启动钩子的
...
如果某个字段不应该被注入，或者它的类型已经被另一个字段占用了，就用 `#[from_ref(skip)]` 退出注入；
...
`#[subscriber]` 处理器通过在载荷之后声明第二个参数来显式启用它；处理器只需要消息本身时就省略这个
...
`()`（因此一个既没有写出上下文类型、也没有接收 [`Ctx` 提取器](#context-fields-as-parameters)的
`#[subscriber]` 处理器，看到的是 `Context<'_>`）。中间件也可以把一个类型化的临时值带给下游的处理器：
借助可写的 key（`FieldMut`），某一层可以 `ctx.set(KEY, value)`，处理器再用 `ctx.context(KEY)` 把它
取回来，例如一个关联 id、或者某一层解析出来的已认证用户，全程不必把它序列化进消息头。上下文每次投递
都重新构造，所以一次投递的值绝不会泄漏到下一次。

## 把上下文字段当作参数 { #context-fields-as-parameters }

字段也可以像 `State<T>` 注入状态组件那样，直接作为处理器的参数到达：`Ctx<K>` 提取器绑定的就是 key
`K` 读到的值。这个 key 实现的是 `ContextField`，一个 `Field` 风格的 trait，只是它还额外写出了自己
读取的上下文类型，并产出一个拥有所有权的值。于是处理器完全不需要 `&mut Context` 参数：
`#[subscriber]` 宏会从签名里第一个 `Ctx` key 推出这个订阅的上下文类型。
...
- 值是拥有所有权的（`ContextField::Value` 是 `'static`）：提取器的值在处理器函数体运行之前就已绑定，
  因此从上下文里借用不是一个选项。产出借用值的 key（比如以 `&str` 形式给出的名字）仍然可以在声明了
  ctx 参数之后通过 `ctx.context(KEY)` 读取。
- 如果签名里同时还有 `&mut Context<'_, C>` 参数，那么每个 `Ctx` key 都必须读取同一个 `C`；这一点由
  编译器通过提取器的 trait 约束来保证。
- 这个推导是语法层面的：宏识别的是字面上的 `Ctx<K>` 形状（任何以 `Ctx` 结尾、带一个类型参数的路径）。
...
- 发出去的消息不会继承这份副本。回复和手动发布都从全新的消息头开始；要给出站消息附加元数据，请改在
...
多次注册会累加，所有匹配的钩子都会运行，运行在投递路径之外一个被跟踪的任务集合里。语义是**至多一次**：
任何钩子运行之前消息就已经结算完毕，所以钩子里发生 panic、或者钩子随进程崩溃一起丢失，都绝不会引发
重新投递。因此不得把「一旦丢失就必须重新投递消息」的工作放进钩子；这类工作应当通过结算结果来表达，交给 Broker
去重试。优雅关闭会把正在执行的钩子排空（受 `shutdown_timeout` 限制）；被中断的关闭则可能直接丢弃它们。

在批量路径上，一个 `Context` 对应的是一**批**消息，因此钩子在整批结算之后才运行。由于一批消息的结算
结果是逐元素的，按结果过滤在这里没有明确含义：只有 `after_settle` 钩子会触发（带过滤的 `after(..)` /
`after_ack` 形式在批量路径上会被忽略）。
