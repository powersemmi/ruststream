...
**应用作用域。** 用 `RustStream::layer` 给整个应用加一层，调用位置在 `with_broker` 之前。在它之后
注册的每一个处理器都会被包裹，既包括直接注册在 Broker 作用域上的处理器，也包括路由器通过
...
**路由器作用域。** 用 `Router::layer` 给某个路由器配置它自己的中间件，它会在该路由器被挂载时包裹
...
其中 `LogLayer` 是下一节手写的那个层，内置的 `layers::TracingLayer` 挂载方式完全相同。
...
`ctx` 就是处理器收到的那个按投递创建的 [`Context`](context.md)，因此一个层可以在处理器读取之前，先
...
当链条要到运行时才能决定（层由配置开关控制，或者被藏在 `dyn` 背后）时，只针对这些处理器显式启用
...
动态的只有那份*列表*。你在运行时把它构建出来，冻结成一个 `DynStack`，得到的就是一个普通的静态
...
执行，最外层的先跑。每个动态层每次调用都要付出一个装箱 future 的代价，而静态层的代价是零，所以请把
