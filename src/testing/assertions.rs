...
impl<T> PublishedAssertions<T> {
    ...
    /// Asserts the most recent published message carries `value` under the header `name` - what a
    /// publish middleware or transform stamped on the way out.
    ///
    /// # Panics
    ///
    ...
    pub fn with_header(self, name: &str, value: impl AsRef<[u8]>) -> Self {
        ...
        let actual = self.last("a header").headers().get(name);
        assert_eq!(
            actual,
            Some(expected),
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "unexpected \"x-app\" header")]
