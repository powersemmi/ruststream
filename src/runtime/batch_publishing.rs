...
//!
//! Generated by `#[subscriber("in", publish("reply-name"))]` on a `&[T]` handler and mounted with
//! [`BrokerScope::include`](super::BrokerScope::include), chaining
//! [`.publisher(..)`](super::IncludeBatchPublishing::publisher) to name the reply publish
...
/// redelivery (at-least-once), while a `.transactional()` wiring never leaves them half-visible.
