...
//! [`.out(Reply, ..)`](super::RouterWith::out) to name the reply publish policy
//! (without it, the statement commits with the broker's default policy); on a
//! [`Router`](super::Router) the same chain ends in `.build()`, which is what commits the
//! registration. At startup the wiring pairs into the live reply sink (the connection plus the
//! reply codec). The destination name comes from the macro; the publisher and codec come from
//! the chain.
...
///
/// Selected by the wiring the include site's chain built: the encoded stack encodes the
...
/// reply is encoded (codec) and *through which* connection it is sent come from the wiring the
...
/// `Wiring` (encoded by the reply stack, or byte-for-byte through a bare publisher) to the
...
pub(crate) struct PublishingHandler<
    Def: PublishingDef,
    DecodeCodec,
    Wiring,
