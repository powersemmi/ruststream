...
/// `batch(..)`, and with the `publish(..)` / `publish_raw(..)` reply forms (not with the
/// batch publishing form yet). An `Out` parameter's attachment is required at the include
/// site: `.publisher(..)` on the plain and batch forms, `.out(..)` on the reply forms
/// (where `.publisher(..)` stays the reply's own attachment).
///
