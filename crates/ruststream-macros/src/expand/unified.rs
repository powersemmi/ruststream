...
fn definition_wiring(
    ...
    // With slots the include-site value is the unit struct, so the reply type and context the
    // mount chain's `.transform(..)` step reads are stated here rather than read off the sealed
    // definition.
