...
// The pool of four on a runtime with worker threads: the workers are pinned to threads of the
// subscription's own, and every delivery crosses from the loop to one of them.
